...
// [CẬP NHẬT] Danh sách Status theo yêu cầu mới
const STATUSES = [
  { id: "ALL", label: "Tất cả", iconClass: "fas fa-list" },
  { id: "PLACED", label: "Đặt hàng", iconClass: "fas fa-file-invoice" },
  { id: "CONFIRMED", label: "Đã thanh toán", iconClass: "fas fa-check-circle" }, // User yêu cầu là Đã thanh toán
  { id: "IN_PROGRESS", label: "Đang tiến hành", iconClass: "fas fa-spinner" },
  { id: "DELIVERED", label: "Đã giao hàng", iconClass: "fas fa-box-open" },
  { id: "CANCELLED", label: "Hủy", iconClass: "fas fa-times" },
  // Đã bỏ: READY_FOR_DELIVERY, OUT_FOR_DELIVERY, REJECTED, FAILED
];
...
const OrderHistoryPage = () => {
  ...
  // Xử lý Hủy đơn (Chỉ cho phép hủy khi mới Đặt hàng)
  const handleCancelOrder = async (orderId) => {
    if (window.confirm("Bạn có chắc muốn hủy đơn hàng này?")) {
      try {
        await orderService.updateStatus(orderId, "CANCELLED");
  ...
  const ActionButtons = ({ order }) => {
    // Chỉ cho phép hủy khi ở trạng thái PLACED (Đặt hàng)
    // Nếu đã CONFIRMED (Đã thanh toán) thường sẽ không cho hủy hoặc cần quy trình hoàn tiền
    const canCancel = ["PLACED"].includes(order.orderStatus);
